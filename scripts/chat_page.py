...
################## Define Functions ##################
# Function to "decrypt" the fake encrypted string
def decrypt_api_key(encrypted_key, password):
    if password != "password":
        return None
    
    # For quick demo we will be using base64 + XOR encryption(encrypter stored in hidden folder) 
    try:
        # Decode base64
        ...
        decrypted_bytes = bytes([decoded_bytes[i] ^     # Cycle through decoded byte
                                 password_bytes[i % len(password_bytes)] # i-th byte from password and cycle back
                                for i in range(len(decoded_bytes))] # Loop through each byte in decoded data [0-255]
                                )
...
def check_authentication():
    ...
    # Create a popup if not authenticated
    if not st.session_state.get("authenticated", False):
        with st.sidebar:
            st.markdown("### 🔐 API Key Authentication")
            
            # Create a simple input field for password
            password = st.text_input(
                "Enter 'password' to access AI Agent", 
                type="password",
                help="Enter 'password' to decrypt the API key"
            )
            
            # Check password only when user enters something
            if password:
                if password == "password":
                    # Fake encrypted API key (base64 encoded real key)
                    fake_encrypted_key = b'FxIYLA1XEwsCAzccDw0FMUMZCwIhByUQJyYXChVcND1CIAsZGScTBQQWNQMtLTgSPlkjMTQVSi8='
                    
                    # Decrypt the API key
                    decrypted_key = decrypt_api_key(fake_encrypted_key, password)
                    
                    if decrypted_key:
                        st.session_state.authenticated = True
                        st.session_state.groq_api_key = decrypted_key
                        st.success("✅ Authentication successful! AI Agent unlocked.")
                        # Force rerun to clear the password input
                        st.rerun()
                    else:
                        st.error("❌ Failed to decrypt API key")
...
# Check authentication - this will run once and handle the flow
authenticated = check_authentication()

# Only start logics when the Password/API key has been authenticated
if authenticated:
